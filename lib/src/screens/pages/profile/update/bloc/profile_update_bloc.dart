...
class ProfileUpdateBloc extends Bloc<ProfileUpdateEvent, ProfileUpdateState> {
  ...
  ProfileUpdateBloc(
    ...
    on<FormSubmit>((event, emit) async {
      ...
      emit(
        state.copyWith(
          responseUpdateProfile: Loading(),
          formKey: formKey
        )
      );

      // PARA QUE APAREZCA El estado de Loading (circle) - Tirar el Back y que quede cargando - ELIMINAR
      Resource responseUpdateProfileRes = await userUseCases.update.run(state.id, state.toUser(), state.image);

      // Issuance of status change - Success/Error
      emit(
        state.copyWith(
          responseUpdateProfile: responseUpdateProfileRes,
          formKey: formKey
        )
      );
    });
